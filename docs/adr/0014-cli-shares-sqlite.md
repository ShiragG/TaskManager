...
- Headless Qt (`QT_QPA_PLATFORM=offscreen` when unset) is enough for `html_to_plain` (`QTextDocument`). Frozen Windows still builds with `console=False`; CLI attaches the parent console before printing.
