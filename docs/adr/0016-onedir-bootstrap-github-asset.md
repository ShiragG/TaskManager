...
On first launch after the existing `.new` helper has swapped the file, the bootstrap unpacks the onedir **beside** the exe (not into `%TEMP%`/`_MEI` on every start). Runtime files go in `data/` next to the exe; `settings.json`, `taskmanager.db`, and `modules/` stay at the install root and must not be overwritten. After a successful extract, leftover `_internal` from older runs is removed. The running bootstrap cannot replace itself: it writes the onedir loader to a temporary name, a short-lived helper waits for the bootstrap PID, moves the loader onto `TaskManager[.exe]`, and **relaunches with the same argv** (GUI or CLI). The Windows helper uses `CREATE_NO_WINDOW` for GUI (not `CREATE_NEW_CONSOLE`, which would flash an empty cmd) and inherits the attached console for CLI; relaunch is `"%TARGET%"` without `start`. After that the exe is a normal onedir (`console=False` + `data/`); CLI no longer pays onefile extract. See [ADR 0015](0015-windows-console-subsystem.md).
