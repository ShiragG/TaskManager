...
Метаданные (номер, описание, срок, цвет, ссылки) живут в SQLite (каталог данных приложения). Имя папки на диске совпадает с номером: при смене номера папка переименовывается автоматически.
